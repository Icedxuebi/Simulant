...
namespace Simulant.Core.Firewall
{
    internal sealed class FirewallService
    {
        ...
        public bool IsEnabled => _onReceiveOriginal != null && _sendHookEnabled;
        ...
        public void ReceiveHookEnable()
        {
            ...
            _host.LogRuntime("收包 Hook 已开启，拦截全部收包。");
        }

        public void ReceiveHookDisable()
        {
            if (_onReceiveOriginal == null || _onReceiveOriginal.Length != 5)
                throw new InvalidOperationException("无法恢复收包，原始数据无效。");
            ...
            _host.LogRuntime("收包 Hook 已关闭，恢复正常收包。");
        }
        ...
        public void SendHookEnable()
        {
            if (_sendHookEnabled)
            {
                _host.LogWarning("发包 Hook 已经开启，跳过操作。");
                return;
            }
            ...
            {
                throw new InvalidOperationException("发包 Hook 开启失败，OnSendPacketFuncPtr 无效。");
            }

            if (_sendHookOriginal != null || _sendHookCave != IntPtr.Zero)
                throw new InvalidOperationException("发包 Hook 开启失败：发包 Hook 状态异常。");
            ...
            if (_sendHookCave == IntPtr.Zero)
                throw new InvalidOperationException("发包 Hook 开启失败，无法分配代码洞。");

            _host.LogVerbose($"Send hook cave @ {(ulong)_sendHookCave:X}");
            ...
            _host.LogRuntime($"发包 Hook 已开启，仅放行心跳包 0x{passOpcode:X4}。");
        }
        ...
        public void SendHookDisable()
        {
            if (!_sendHookEnabled)
            {
                _host.LogWarning("发包 Hook 当前未开启，跳过关闭操作。");
                return;
            }
            ...
            if (target == IntPtr.Zero)
            {
                throw new InvalidOperationException("发包 Hook 关闭失败：OnSendPacketFuncPtr 无效。");
            }

            if (_sendHookOriginal == null || _sendHookOriginal.Length != SendHookLength)
            {
                throw new InvalidOperationException("发包 Hook 关闭失败：原始字节无效。");
            }
